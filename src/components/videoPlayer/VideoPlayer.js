...
const VideoPlayer = (props) => {
  ...
  const fetchComments = () => {
    ...
    fetch(
      `https://www.googleapis.com/youtube/v3/commentThreads?key=${apiKey}&textFormat=plainText&part=snippet&videoId=${id.id}&maxResults=${commentsAmmount.current}`
    )
  ...
  const fetchRelatedVideos = (id) => {
    fetch(
      `https://www.googleapis.com/youtube/v3/search?part=snippet&relatedToVideoId=${id}&type=video&key=${apiKey}&maxResults=${relatedVidAmmount.current}`
    )
      .then((response) => response.json())
      .then((responseData) => {
  ...
  const setScreenSize = () => {
    window.innerWidth < 1000
      ? setShowButtonMoreRelated(true)
      : setShowButtonMoreRelated(false);
  };
    ...
    <>
      ...
      <div className={`${classes.VideoPlayer} `}>
          ...
          <div className={classes.playerSection}>
            ...
            <div className={classes.videoDetailsContainer}>
              ...
              <div className={classes.channelInfoBox}>
                <div className={classes.profileImgBox}>
                  <img
                    ...
                    src={`${currentVideoDetails.profileImg}`}
                    alt="profile avatar"
                  ></img>
                </div>
                <div className={classes.channelDetailsBox}>
                  ...
                  <div className={classes.subscriberCount}>
                    <SubscriberCounter
                      subscriberAmount={currentVideoDetails.subscriberCount}
                    />
                  </div>
                </div>
              </div>
              <div className={classes.videoDescriptionBox}>
                <div
                  ref={descriptionRef}
                  className={`${classes.videoDescriptionText} ${
                    isBtnActive && classes.videoDescriptionTextActive
                  }`}
                >
                  <Linkify>{currentVideoDetails.description}</Linkify>
                </div>
                {showButtonMore && (
                  <ButtonShowMore
                    isBtnActive={isBtnActive}
                    callFunc={toggleClass}
                  />
                )}
              </div>
            </div>
        ...
        <div className={classes.relatedVideosSection}>
          <RelatedVideoCard
            onSetLoader={setLoaderHandler}
            relatedVideos={relatedVideos}
            onFetchRelatedVideos={fetchRelatedVideos}
            onChangeVideoDetails={props.onChangeVideoDetails}
          />
          {showFetchMoreLoader && (
            <Oval
              height={35}
          ...
          {showButtonMoreRelated && (
            <button
              onClick={showMoreRelated}
              className={classes.relatedVideoBtn}
            >
              POKAŻ WIĘCEJ
            </button>
          )}
