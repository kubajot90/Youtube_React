...
const VideosSection = (props) => {
  ...
  const fetchVideos = () => {
    ...
    clearVideosDetails();
    fetch(
      `https://youtube.googleapis.com/youtube/v3/search?key=${apiKey}&videoEmbeddable=true&order=viewCount&q=${search}&type=video&part=snippet&maxResults=${videosAmount.current}`
    )
  ...
  const setChannelsIdArr = () => {
    let idsArr = [];
    videos.forEach((video) => idsArr.push(video.snippet.channelId));
  ...
  const fetchProfileImgUrl = () => {
    ...
    channelIds.forEach((id) => {
      fetch(
        `https://youtube.googleapis.com/youtube/v3/channels?key=${apiKey}&part=snippet&part=statistics&id=${id}`
      )
        .then((response) => response.json())
        .then((responseData) => {
          const obj = {
        ...
        .then(() => {
          ...
          isSearch &&
            setLoaderBarWidth((prev) => prev + 45 / videosAmount.current);
        })
        .catch((error) => {
          console.error("Error:", error);
        });
  ...
  useEffect(() => {
    if (
      profilesImgObj.length === videos.length &&
      videos.length === videosAmount.current
    ) {
      createCardsFunc();
    }
  }, [profilesImgObj]);
